...
macOS, Linux, Windows 공통 실행은 `scripts/replace-template-variables.mjs`를 권장한다. Node.js만 있으면 `node scripts/replace-template-variables.mjs --root . --variables-file scripts/template-variables.example.yaml`로 Dry Run을 확인하고, 실제 적용 시 `--apply`를 붙인다. Windows PowerShell 환경에서는 기존 `scripts/replace-template-variables.ps1`도 사용할 수 있다.

## 초기 개발 문서 입력 공간

각 템플릿에는 inputs/ 폴더가 포함된다. 사용자는 구현 전에 초기 개발 문서, 기획 메모, 요구사항 초안, 화면 스케치, 레퍼런스 링크를 이곳에 넣을 수 있고, Agent는 해당 문서를 검토한 뒤 사용자 인터뷰를 진행한다.
