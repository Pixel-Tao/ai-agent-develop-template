...
- `skills/catalog.yaml`과 `scripts/skills.sh` 기반 Skill 검색/추가 흐름
...
6. Agent가 `INIT.md`, `AGENTS.md`, `manifest.yaml`, `skills/catalog.yaml`, `harness/`를 읽고 초기 인터뷰, Skill 구성, 작업 상태 정리를 진행합니다.
...
| `skills/catalog.yaml` | 초기화 중 검색 가능한 Skill 후보 목록 |
| `scripts/skills.sh` | Skill 목록/검색/추가 helper |
...
| `sh scripts/skills.sh search <query>` | 생성 프로젝트에서 Skill 후보 검색 |
| `sh scripts/skills.sh add --from-catalog <id>` | 생성 프로젝트에 catalog 기반 Skill 추가 |
...
| `npm run test:skills` | 생성 프로젝트 Skill 검색/추가 테스트 |
