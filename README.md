...
- `skills/skills-sh-recommendations.yaml` 기반 기술 스택 감지와 skills.sh 검색/설치 후보 구성
- `mcp/` 기반 MCP 서버 후보 검토, 승인, 접근 범위 통제
...
6. Agent가 `INIT.md`, `AGENTS.md`, `manifest.yaml`, `skills/skills-sh-recommendations.yaml`, `mcp/`, `harness/`를 읽고 초기 인터뷰, 기술 스택 감지, skills.sh 기반 Skill 구성, MCP 검토, 작업 상태 정리를 진행합니다.
...
| `skills/skills-sh-recommendations.yaml` | 기술 스택 감지 규칙, skills.sh 검색어, 추천 source 목록 |
| `skills/selected-skills.md` | 초기화 중 검토한 Skill 후보와 적용/제외 사유 |
| `mcp/` | MCP 정책, 후보 서버, selection log |
...
| `npm run test:skills` | 템플릿별 기술 스택 기반 skills.sh 추천 구성 테스트 |
| `npm run test:mcp` | 모든 템플릿 MCP 정책과 후보 서버 검증 |
