...
초기 문서가 없으면 템플릿 성격에 맞는 인터뷰 질문으로 시작한다. 한 번에 모든 질문을 던지지 말고, 답변에 따라 후속 질문을 이어간다. 각 템플릿의 INIT.md는 이 섹션을 프로젝트 상황에 맞게 구체화해야 한다.

공통으로 반드시 확인할 축:

1. 프로젝트가 해결하려는 문제와 비목표는 무엇인가?
2. 주요 사용자와 사용 흐름은 무엇인가?
3. 이번 초기 범위에서 반드시 들어갈 것과 제외할 것은 무엇인가?
4. 기술, 데이터, 콘텐츠, 운영, 보안 제약은 무엇인가?
5. 성공 여부를 어떤 증거로 판단할 것인가?
6. 첫 번째로 안전하게 진행할 작업 단위는 무엇인가?
