# Initialization Prompt: MVP Prototype

당신은 이 프로젝트를 MVP Prototype 템플릿 기준으로 초기화하는 AI Agent다.
...
아이디어 검증과 빠른 프로토타입 개발을 위한 경량 구조. 프로젝트에 이미 존재하는 파일은 덮어쓰지 말고, 차이가 있으면 병합 필요 항목으로 기록한다.
...
초기화 작업을 시작하기 전에 docs/project-discovery-interview.md를 사용해 사용자의 목표, 대상 사용자, 핵심 콘텐츠, 기술 방향, 성공 기준, 제외 범위를 인터뷰한다. 사용자가 모호하게 답한 항목은 확정하지 말고 assumptions 또는 open questions로 분리한다.
