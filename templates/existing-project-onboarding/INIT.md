...
10. skills/catalog.yaml
11. scripts/skills.sh

## Skill 구성 초기화

초기 인터뷰와 문서 확인 후, Agent는 개발에 필요한 Skill이 현재 프로젝트에 충분한지 확인한다.
...
2. 작업 성격에 맞는 Skill 후보를 검색한다.

   ```bash
   sh scripts/skills.sh search api
   sh scripts/skills.sh search security
   sh scripts/skills.sh search test
   ```

3. 필요한 Skill이 catalog에 있으면 추가한다.

   ```bash
   sh scripts/skills.sh add --from-catalog <skill-id>
   ```

4. catalog에 없는 프로젝트 전용 Skill이 필요하면 scaffold한다.

   ```bash
   sh scripts/skills.sh add --id <skill-id> --name "Skill Name" --description "이 Skill이 수행할 작업"
   ```

5. 추가한 Skill은 `skills/skills-index.yaml`, `skills/selected-skills.md`, `docs/09_agent_state/run-log.md`에 근거와 함께 기록한다.

Skill을 추가할 때는 실제 작업에 필요한 경우로 제한한다. 불확실한 Skill은 추가하지 말고 후보와 이유를 assumptions 또는 run log에 남긴다.
...
이 템플릿은 이미 존재하는 코드베이스에 Agent를 투입할 때 사용한다. 인터뷰는 새 기능 정의보다 기존 구조, 변경 금지 영역, 안전한 분석 순서를 파악하는 데 집중한다. 사용자가 모르는 항목은 추정하지 말고 discovery 문서의 unknowns로 남긴다.

1. 이 시스템은 현재 어떤 업무나 사용자 흐름을 지원하는가?
2. 가장 자주 변경되는 영역과 가장 변경하면 위험한 영역은 어디인가?
3. 반드시 보존해야 하는 기존 동작, API, 데이터, 화면, 배포 방식은 무엇인가?
4. 코드 스타일, 브랜치 전략, 리뷰 규칙, 릴리즈 규칙처럼 이미 지켜야 하는 컨벤션이 있는가?
5. 테스트, 빌드, 린트, 배포 명령은 무엇이며 신뢰도는 어느 정도인가?
6. 문서가 오래되었거나 실제 코드와 다를 가능성이 큰 영역은 어디인가?
7. 승인 없이 삭제, 리팩터링, API 변경을 하면 안 되는 영역은 무엇인가?
8. 현재 가장 큰 위험은 장애, 성능, 보안, 유지보수성, 테스트 부족 중 무엇인가?
9. Agent가 첫 번째로 작성해야 할 산출물은 repository map, dependency map, risk register 중 무엇인가?
10. 첫 번째 변경 작업을 한다면 영향 범위가 가장 작고 검증 가능한 작업은 무엇인가?

## 4. 프로젝트 정의 산출물

인터뷰 결과를 다음 문서에 정리한다.

- docs/01_context/project-definition-brief.md
