...
AI Agent는 Existing Project Onboarding 템플릿 기준으로 프로젝트 구조, 현재 상태, 위험 요소를 먼저 파악한 뒤 요청된 작업을 수행한다. 기존 구조를 먼저 분석한다. 기존 코드 스타일과 컨벤션을 보존한다. 변경 전에 영향 범위를 기록한다. 삭제, 리팩터링, API 변경은 별도 승인 대상으로 표시한다. 테스트가 없는 영역은 test-inventory.md와 risk-register.md에 기록한다. 모르는 내용은 unknowns.md에 기록하고 추정한 내용은 assumptions.md에 기록한다.
