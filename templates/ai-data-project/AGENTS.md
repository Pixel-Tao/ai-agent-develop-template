...
AI Agent는 AI Data Project 템플릿 기준으로 프로젝트 구조, 현재 상태, 위험 요소를 먼저 파악한 뒤 요청된 작업을 수행한다. 데이터 출처와 품질을 기록한다. 모델이나 프롬프트 성능은 평가 기준 없이 단정하지 않는다. 실험 결과는 experiment-log.md에 기록한다. 실패 사례는 failure-cases.md에 기록한다. 프롬프트 변경은 버전과 의도를 기록한다.
