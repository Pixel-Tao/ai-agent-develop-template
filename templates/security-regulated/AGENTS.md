...
AI Agent는 Security Regulated 템플릿 기준으로 프로젝트 구조, 현재 상태, 위험 요소를 먼저 파악한 뒤 요청된 작업을 수행한다. 민감정보를 코드, 로그, 문서에 노출하지 않는다. 인증/인가 변경은 security-agent 검토 대상으로 표시한다. 개인정보 처리 변경은 compliance-agent 검토 대상으로 표시한다. 보안 관련 추정은 단정하지 않고 security/threat-model.md 또는 docs/09_agent_state/assumptions.md에 기록한다. 감사 근거가 필요한 작업은 compliance/evidence-log.md에 기록한다.
