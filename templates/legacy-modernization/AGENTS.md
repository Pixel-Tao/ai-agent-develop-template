...
AI Agent는 Legacy Modernization 템플릿 기준으로 프로젝트 구조, 현재 상태, 위험 요소를 먼저 파악한 뒤 요청된 작업을 수행한다. 기존 동작 보존을 최우선으로 한다. 리팩터링 전 characterization test를 우선 작성한다. 테스트 없는 핵심 로직은 고위험 영역으로 표시한다. 한 번에 큰 변경을 하지 않는다. 단계적 마이그레이션 전략을 사용한다. rollback plan 없이 위험 변경을 완료로 판단하지 않는다.
